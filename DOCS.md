...
- 使用真实 MEXC 公共行情完成 30 分钟持续运行，并完成三交易所同时运行、分所状态、实时深度和
...
- HTTPX：MEXC、OKX 与 Binance 公共 REST；
- websockets：三所公共深度；protobuf 仅用于 MEXC，OKX/Binance 使用严格 JSON 增量序列；
...
- 已实现中文实时机会、三腿明细、历史和健康状态页面；机会、近似机会和连接均标注交易所，实时与历史
  支持交易所及锚定资产组合过滤，状态页按 MEXC/OKX/Binance 分栏；
- 已实现声音提示、重连覆盖和降级状态展示；
...
- 已运行 `tri-arb doctor`，配置、SQLite、Protobuf 及三交易所公共 REST 检查全部通过；
- 已使用 MEXC 公共行情连续观测 30 分 12 秒，并在中点受控重启，验证 REST 周期、WebSocket 恢复、
  动态订阅、SQLite 写入和前端 snapshot 恢复；
- 已同时运行三所真实公共行情，验证 4,713 个市场、8,516 条路径及分所状态；

...
>
> 最后更新：2026-07-21（里程碑 3：完成 MEXC 公共 REST 与 WebSocket 协议层）

## 1. 当前状态

仓库已完成里程碑 2，并正在实施里程碑 3。行情轮询编排、扫描流水线和机会接口尚未实现。当前已有能力为：
...
**状态：进行中（公共 REST、WebSocket、Protobuf 与订阅协调已完成，行情运行时编排待完成）。**

- 已实现 REST 元数据、时间校准、全量 book ticker、退避和单市场隔离结果；健康状态待扫描运行时接线；
- 已固定并生成公共只读 Protobuf，解析 20 档 partial-depth，并由 CI 检查生成代码漂移；
- 已实现 Top 20 订阅协调、两连接分片、PING、重连、主动轮换和连接/订阅代次；
- 已使用录制的脱敏公共二进制帧完成适配器契约测试，REST 响应夹具随行情运行时补齐。

...
>
> 最后更新：2026-07-21（增加 MEXC USDT/USDC/USD1 多锚定扫描）
...
从全部可交易现货市场中寻找以 USDT、USDC 或 USD1 为起点和终点的三腿闭环，例如
...
2026-07-21 的公共元数据观测到 736 条 USDT、638 条 USDC 和 238 条 USD1 有向三角路径；这些数字
仅用于容量设计，元数据刷新后以实时枚举为准。短名单总上限仍为 20，并按当前存在路径的锚定资产
公平分配基础配额，再以剩余全局收益排名补足，避免单一锚定资产完全挤占深度订阅。
...
- `GET /api/status`：扫描阶段、市场/边/路径数、REST 与价格保护参考价年龄、WebSocket 连接和订阅数、
  最近错误，以及最新诊断快照；

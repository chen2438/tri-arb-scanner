...
>
> 最后更新：2026-07-21（完成 MEXC + OKX + Binance + Bybit 独立扫描）
...
- 使用真实 MEXC 公共行情完成 30 分钟持续运行，并完成四交易所同时运行、分所状态、实时深度和
...
Tri-Arb Scanner 是一个本地优先、可审计的**三角套利机会扫描器**。当前分别扫描 MEXC、OKX、Binance 与 Bybit 现货
...
- 只使用 MEXC、OKX、Binance 与 Bybit 公共 REST/WebSocket 行情，不需要 API Key；
...
全市场 REST 最优报价若交易所未提供来源时间，领域模型会把来源时间显式记为不可用，仅保留本地接收
时间并限制为只参与广筛。默认 1 秒轮询时，超过 5 秒未刷新或接收时间来自未来的报价会从广筛输入中
移除，对应交易所退出 `ready`；最终机会始终要求三腿带来源时间的新鲜 WebSocket 深度确认。
...
应用默认同时启动 MEXC、OKX、Binance 与 Bybit。统一扫描循环对每个交易所分别执行多锚定广筛、分别选取最多
20 条路径并回写各自的深度订阅，再把四边的精确确认结果合并到同一个生命周期、SQLite、REST 和 WebSocket
事件流。路径 ID 含交易所前缀，因此一个交易所的缺失、关闭或同名市场不会覆盖另一个交易所的机会。
聚合诊断统计四边全部路径和最多 80 条短名单，API 状态同时保留总计与 `exchanges` 分交易所明细。
...
Bybit 适配器只访问 V5 `market` 公共端点：`instruments-info?category=spot`、`tickers?category=spot`、
`time`、逐市场 `price-limit` 与 `orderbook`，不使用认证头或账户接口。现货 `basePrecision` 作为数量
量子，`minOrderAmt` 作为最小报价金额，`maxMarketOrderQty` 作为市场单数量上限。由于公开费率同时存在
普通现货 10 bps 和特殊交易区 20 bps，未读取账户费率时默认按 20 bps 计算且配置不得低于该值。

Bybit 的每条候选腿必须同时具备新鲜公共 `price-limit` 与 `orderbook.200` 重建盘口；价格限制直接使用
交易所计算的 `buyLmt/sellLmt/ts`，不自行推导公式。WebSocket 使用顶层 `cts` 作为撮合来源时间，使用
`u/seq` 拒绝乱序；单次订阅最多发送 10 个 topic，两条连接各最多维护 30 个市场。临时单边或空 snapshot
只隔离对应市场并忽略其 delta，不清空同分片的健康盘口。Bybit 使用 `BYBIT|` 前缀独立建图和扫描。
...
`confirmed_capacity` 使用同一三腿模拟器：先以已经通过确认的默认模拟规模作为下界，逐次将规模翻倍，
直到三腿 20 档深度、价格保护或市场最大约束使模拟失败并形成有限上界，再做最多 32 次二分。结果向下
取整到 `0.01` 锚定资产，并明确表示“20 档内已确认容量”，不能外推到未订阅深度。
...
Bybit instruments/tickers --> Bybit 市场图/独立 Top 20 -> orderbook.200 + price-limit
                                                        |
                                                        +------> 同一确认结果汇聚点
```
...
- HTTPX：MEXC、OKX、Binance 与 Bybit 公共 REST；
- websockets：四所公共深度；protobuf 仅用于 MEXC，OKX/Binance/Bybit 使用严格 JSON 增量序列；
...
- `market_data` 与各 `exchange/*/depth`：快照版本、时序、新鲜度和订阅状态；
...
- 标准 wheel 构建必须执行锁定的前端生产构建并将 `frontend_dist` 包入 `tri_arb`；运行时优先读取包内
  资源，editable 开发安装才回退到仓库 `frontend/dist`，禁止发布只有 API、没有网页的残缺 wheel；
- 前端产物只在标准 wheel 构建钩子完成后动态加入打包清单；editable 安装不得要求预先存在
  `frontend/dist`，以保证纯 Python 的后端开发与 CI 环境可以独立安装项目；
- 前端只展示后端 Decimal 字符串，不自行重复收益计算；
- 活跃机会表展示路径、净收益、预估利润、确认容量和行情年龄；
- 展开行展示三腿方向、市场、档位均价、输入/输出、费用、dust 和时间；
- 价格保护参考价使用交易所无关标签展示；不得把 Binance 交易所计算参考价误标为 MEXC 的 5 分钟均价；
- 历史页展示开启、峰值与关闭原因；状态区展示 REST 年龄、路径数、订阅数、连接与最近错误；
- 历史交易所/锚定资产筛选由 REST 查询执行并在筛选变化时重置游标，不能只过滤浏览器已加载的当前页；
...
- `tri-arb doctor`：逐项检查配置、SQLite 临时写入、本地生成后解码的 Protobuf 固定消息，以及 MEXC
  ping/time/exchangeInfo/book ticker、OKX time/instruments/tickers、Binance time/exchangeInfo/tickers 和
  Bybit time/instruments/tickers；不创建 API Key、不访问私有接口，任一启用交易所检查失败时返回退出码 1。

服务缺少完整元数据或首份全市场报价时可以存活但未就绪；全市场报价超过其新鲜度窗口时退出
`ready` 且不再进入广筛。行情任务明确报错或 WebSocket 中断时进程继续运行，状态转为 `degraded` 并
关闭依赖该行情的机会。

收到 `Ctrl+C`/进程关闭请求后，应用先设置统一停止事件并给予后台任务 2 秒正常清理窗口；仍在等待
交易所 HTTP、重试退避或 `Retry-After` 的任务会被主动取消。取消后的 `finally` 清理另有 1 秒等待窗口，
超时任务会收到第二次取消且应用停止等待。正常取消仍会关闭 HTTP/WebSocket、SQLite 写队列及活跃
生命周期；异常清理不能重新引入无界的应用级等待。
...
完整 snapshot 为准；sequence 跳跃时客户端使用允许发送的应用关闭码 `4000` 主动重连，不能使用浏览器
保留且禁止客户端发送的 `1012`。v0.1 不提供跨进程事件补发。
...
- 已实现中文实时机会、三腿明细、历史和健康状态页面；机会、近似机会和连接均标注交易所，实时与历史
  支持交易所及锚定资产组合过滤，状态页按 MEXC/OKX/Binance/Bybit 分栏；
- 已实现声音提示、重连覆盖和降级状态展示；
...
- `tri-arb doctor` 已覆盖四交易所公共 REST；本轮 MEXC、OKX、Bybit 检查通过，完整运行因 Binance
  返回限频并要求长时间 `Retry-After` 而失败，未将其记录为四所全部通过；为避免单一端点长期阻塞
  后续诊断，每项 doctor 检查独立限制为 10 秒，超时会明确标为失败并继续检查其他交易所；
- 已使用 MEXC 公共行情连续观测 30 分 12 秒，并在中点受控重启，验证 REST 周期、WebSocket 恢复、
  动态订阅、SQLite 写入和前端 snapshot 恢复；
- 已同时启动四所真实公共行情，验证独立路径、公共价格限制、实时深度及分所状态；本轮 MEXC、OKX、
  Bybit 达到 `ready`，Binance 公共 API 返回限频并保持初始化，未将该次运行记为四所同时就绪；
...
.venv/bin/pip wheel . --no-deps --wheel-dir /tmp/tri-arb-wheel
.venv/bin/python scripts/check_wheel_contents.py /tmp/tri-arb-wheel/*.whl

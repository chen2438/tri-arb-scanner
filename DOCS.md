...
>
> 最后更新：2026-07-21（完成 MEXC + OKX 独立扫描与统一展示）
...
Tri-Arb Scanner 是一个本地优先、可审计的**三角套利机会扫描器**。当前分别扫描 MEXC 与 OKX 现货
...
- 只使用 MEXC 与 OKX 公共 REST/WebSocket 行情，不需要 API Key；
...
缓冲上限、控制响应错误或 24 小时服务端断开均触发整条分片重连并等待新快照。当前连接组件已经完成，
尚未接入应用行情服务。

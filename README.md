...
**MEXC 与 OKX 现货市场的独立只读扫描**，不包含下单能力，也不拼接跨交易所的三条腿。
...
- 已完成 MEXC 公共 REST/Protobuf 深度与 OKX 公共 REST/增量深度，两所分别维护 Top 20 候选；
- 已完成 Decimal 三角路径广筛、深度确认、容量、机会生命周期和 SQLite 审计重放；
- 已完成双交易所统一确认、只读 REST/WebSocket 机会接口及应用运行时；
...
配置模板见 `.env.example`。当前版本只接受回环监听地址；MEXC 与 OKX 正式 endpoint 必须使用
HTTPS/WSS。OKX 默认启用，可用 `TRI_ARB_OKX_ENABLED=false` 关闭。

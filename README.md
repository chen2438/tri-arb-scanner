...
**MEXC、OKX 与 Binance 现货市场的独立只读扫描**，不包含下单能力，也不拼接跨交易所的三条腿。
...
- 已完成 MEXC、OKX 与 Binance 公共 REST/实时深度，三所分别维护 Top 20 候选；
- 已完成 Decimal 三角路径广筛、深度确认、容量、机会生命周期和 SQLite 审计重放；
- 已完成三交易所统一确认、只读 REST/WebSocket 机会接口及应用运行时；
...
配置模板见 `.env.example`。当前版本只接受回环监听地址；三家交易所正式 endpoint 必须使用
HTTPS/WSS。OKX 与 Binance 默认启用，可分别用 `TRI_ARB_OKX_ENABLED=false`、
`TRI_ARB_BINANCE_ENABLED=false` 关闭。

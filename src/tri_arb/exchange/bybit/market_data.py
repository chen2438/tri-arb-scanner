"""Independent async market-data service for public Bybit spot data."""
...
from tri_arb.exchange.bybit.metadata import NormalizedBybitInstruments
from tri_arb.exchange.bybit.rest import BybitRestClient, NormalizedBybitTickers
...
class BybitMarketDataService:
    """Own Bybit REST schedules, depth subscriptions, and coherent snapshots."""
    ...
    def __init__(
        ...
        *,
        ...
        self._rest = rest_client or BybitRestClient(
        ...
        self._shards = (
            BybitDepthWebSocketShard(
                settings.bybit_ws_url, 0, self._accept_depth, on_status=self._accept_ws_status
            ),
            BybitDepthWebSocketShard(
                settings.bybit_ws_url, 1, self._accept_depth, on_status=self._accept_ws_status
            ),
        )
    ...
    async def refresh_metadata(self) -> NormalizedBybitInstruments:
        ...
        log_event(
            LOGGER,
            "bybit_market_data.metadata_refreshed",
    ...
    async def refresh_tickers(self) -> NormalizedBybitTickers:
    ...
    async def _accept_depth(self, update: DepthUpdate) -> None:
        async with self._lock:
            ...
            if previous is not None and tuple(
                int(part) for part in update.book.version.split(":", 1)
            ) <= tuple(int(part) for part in previous.book.version.split(":", 1)):
                return

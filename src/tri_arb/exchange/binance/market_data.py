"""Independent async market-data service for public Binance Spot data."""
...
from tri_arb.exchange.binance.metadata import NormalizedBinanceExchangeInfo
from tri_arb.exchange.binance.rest import BinanceRestClient, NormalizedBinanceTickers
...
class BinanceMarketDataService:
    """Own Binance REST schedules, depth/reference streams, and coherent snapshots."""
    ...
    def __init__(
        ...
        *,
        ...
        self._references: dict[str, PriceReference] = {}
        ...
        self._last_reference_ms: int | None = None
        ...
        self._shards = (
            BinanceDepthWebSocketShard(
                ...
                0,
                self._rest.depth_snapshot,
                self._accept_depth,
                on_status=self._accept_ws_status,
                on_reference=self._accept_reference,
            ),
            BinanceDepthWebSocketShard(
                settings.binance_ws_url,
                1,
    ...
    async def refresh_metadata(self) -> NormalizedBinanceExchangeInfo:
        ...
        async with self._lock:
            ...
            self._references = {
                key: value for key, value in self._references.items() if key in symbols
            }
        ...
        log_event(
            LOGGER,
            "binance_market_data.metadata_refreshed",
    ...
    async def refresh_tickers(self) -> NormalizedBinanceTickers:
    ...
    async def reconcile_depth(self) -> SubscriptionPlan:
        async with self._lock:
            ...
            self._references = {
                symbol: value for symbol, value in self._references.items() if symbol in current
            }
            self._plan = plan
    ...
    async def _accept_reference(self, update: ReferenceUpdate) -> None:
        async with self._lock:
            expected = self._plan.shards[update.shard_id]
            status = self._websocket_statuses.get(update.shard_id)
            if (
                update.reference.symbol not in expected
                or status is None
                or status.state is not WebSocketState.CONNECTED
                or update.connection_generation != status.connection_generation
            ):
                return
            previous = self._references.get(update.reference.symbol)
            if (
                previous is not None
                and previous.source_time_ms is not None
                and update.reference.source_time_ms is not None
                and update.reference.source_time_ms <= previous.source_time_ms
            ):
                return
            self._references[update.reference.symbol] = update.reference
            self._last_reference_ms = self._now_ms()
    ...
    async def _accept_ws_status(self, status: WebSocketStatus) -> None:
        async with self._lock:
            ...
            if (
                ...
                self._references = {
                    symbol: value
                    for symbol, value in self._references.items()
                    if symbol not in symbols
                }
    ...
    async def snapshot(self) -> MarketDataSnapshot:
        async with self._lock:
            status = MarketDataStatus(
                ...
                price_reference_count=len(self._references),
                ...
                last_price_reference_ms=self._last_reference_ms,
            ...
            return MarketDataSnapshot(
                ...
                routes=self._routes,
                tickers=dict(self._tickers),
                price_references=dict(self._references),
                price_limits={},

...
const opportunity: Opportunity = {
  id: "life-1", exchange: "MEXC", route_id: "USDT-BTC-ETH-USDT", state: "active",
...
const exchangeStatus = { exchange: "MEXC", phase: "ready", ready: true, market_count: 2040, route_count: 736, ticker_count: 2040, price_reference_count: 6, market_activity_count: 2040, core_market_count: 30, core_route_count: 48, depth_book_count: 6, subscription_count: 6, rest_metadata_age_ms: 1000, rest_clock_age_ms: 2000, rest_ticker_age_ms: 250, rest_price_reference_age_ms: 500, rest_market_activity_age_ms: 800, last_error: null, websocket_connections: [{ exchange: "MEXC", shard_id: 0, state: "connected", generation: 2, subscription_count: 6, error: null }] };
const okxConnection = { exchange: "OKX", shard_id: 0, state: "connected", generation: 1, subscription_count: 10, error: null };

const scanner = {
  live: { connection: "connected" as const, sequence: 4, awaitingSnapshot: false, resyncRequired: false, opportunities: [opportunity], status: { phase: "scanning", ready: true, exchanges: [exchangeStatus], market_count: 2040, route_count: 736, ticker_count: 2040, price_reference_count: 6, market_activity_count: 2040, core_market_count: 30, core_route_count: 48, depth_book_count: 6, subscription_count: 6, active_opportunity_count: 1, diagnostics: { updated_at_ms: 1000, total_route_count: 736, priced_route_count: 730, positive_route_count: 3, shortlisted_route_count: 20, depth_confirmed_count: 1, best_estimated_return_bps: "18", rejection_counts: { price_protection: 2, missing_current_depth: 17 }, near_misses: [{ exchange: "MEXC", route_id: "near", assets: ["USDT", "A", "B", "USDT"], net_return_bps: "15", estimated_profit: "0.15", confirmed_capacity: "250", market_age_ms: 80, leg_skew_ms: 10 }], rolling_confirmed_sample_count: 12, rolling_max_net_return_bps: "19", rolling_buckets: { negative: 4, "0_to_5": 2, "5_to_10": 2, "10_to_threshold": 4, opportunity: 0 } }, rest_metadata_age_ms: 1000, rest_clock_age_ms: 2000, rest_ticker_age_ms: 250, rest_price_reference_age_ms: 500, rest_market_activity_age_ms: 800, last_scan_at: "2026-07-21T12:00:01.000Z", last_error: null, websocket_connections: [...exchangeStatus.websocket_connections, okxConnection] } },
  config: { anchor_asset: "USDT", anchor_assets: ["USDT", "USDC", "USD1"], notional: "100", min_net_return_bps: "20", safety_buffer_bps: "5", depth_levels: 20, okx_enabled: true, okx_taker_commission: "0.0015", binance_enabled: true, binance_taker_commission: "0.001", bybit_enabled: true, bybit_taker_commission: "0.002" },
...
test("navigates to history and system status", () => {
  ...
  expect(screen.getByText("MEXC 分片 1")).toBeInTheDocument();
  expect(screen.getByText("OKX 分片 1")).toBeInTheDocument();
  expect(screen.getByText("长期核心市场")).toBeInTheDocument();
...
test("shows exchange identity and filters opportunities by venue", () => {
  ...
  expect(screen.getByRole("option", { name: "BYBIT" })).toBeInTheDocument();
